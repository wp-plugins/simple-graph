 ...
 * pjm_graph.php
 *
 * @author Mark Dennehy <mark.dennehy@gmail.com
 * @package default
 */
...
Author: Mark Dennehy
Author URI: http://stochasticgeometry.ie
Licence: GPL2
*/
...
define('PJM_GRAPH_PLUGIN_PATH',
	ABSPATH .
	'/wp-content/plugins/' .
	dirname(plugin_basename(__FILE__))
);

define('PJM_GRAPH_PLUGIN_URL',
	get_bloginfo('wpurl') .
	'/wp-content/plugins/' .
	dirname(plugin_basename(__FILE__))
);
 ...
 *
 * @param unknown $args
 * @param integer $number (optional) Graph ID number
 */
function widget_pjm_graph_widget($args, $number = 1) {
    ...
    if (!is_array($options[$number]))
        $options[$number] = array(  'title'           => '',
                                    ...
                                    'line_col'        => '0000FF',
                                    'bg_line_col'     => 'CCCCFF',
                                    'trend_line_col'  => '88FF88',
                                    'target_line_col' => 'FF0000',
                                    'date_fmt'        => 'y/m/d',
                                    'show_text'       => TRUE,
                                    'show_title'      => TRUE,
                                    'show_trend'      => FALSE,
    ...
    $tags = null;
    if (($options[$number]['show_title'] && 
        (strpos($options[$number]['title'], "%") !==FALSE) ) ||
        ($options[$number]['show_text']) && 
        (strpos($options[$number]['text'], "%") !==FALSE)) {
    ...
    if ($title!=null) {
        echo $before_title;
        echo pjm_graph_tags($title, $tags);
        echo $after_title;
    }
    ...
    pjm_graph(  $number,
                ...
                $options[$number]['show_trend'],
                $options[$number]['show_target'],
                FALSE,
                FALSE,
                FALSE,
                $options[$number]['user_id'],
                $options[$number]['table_id'],
                FALSE,
                $options[$number]['gchart'] );

    echo "</p>";

    if ($options[$number]['show_text'])
        echo pjm_graph_tags($options[$number]['text'], $tags);
...
/**
 * Get the various tags for substitution into the Widget's text from 
 * the dataset (%CURRENT, %HIGH, %LOW, etc)
 *
 * @param Integer $uid User ID
 * @param Integer $tid Graph ID
 * @return Array 
 */
...
/**
 * Substitute values for tags in the widget text (%CURRENT, %HIGH, 
 * %LOW, etc...)
 *
 * @param String $string Widget text before substitution
 * @param Array $tag_values Values to substitute for tags in an array
 * @return String The string with the values substituted for the placeholder tags
 */
...
/**
 * Util function to format a colour string read from the user
 *
 * @param String $col User input of a colour string
 * @return String Sanitised version of input
 */
 ...
 * @param unknown $trend           (optional)
 * @param unknown $target          (optional)
 * @param unknown $ytd             (optional)
 * @param unknown $lm              (optional)
 * @param unknown $wkly            (optional)
 * @param unknown $user_id         (optional)
 * @param unknown $table_id        (optional)
 * @param unknown $only_return_tag (optional)
 * @param unknown $use_gchart      (optional)
 * @return unknown
 */
